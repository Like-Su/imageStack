...
- 图片详情工具栏的「短链分享」与相册详情的「分享相册」打开 Element Plus 分享弹窗，可选择 1/7/30 天或永久有效（默认 7 天）、生成和复制短链、查看最近链接并撤销。需要 `asset:share` 权限，管理员可在用户/角色授权中配置。
- `/s/:token` 为公开落地页，访客可分页预览图片和查看原图；点击「登录后保存」会带安全站内返回地址进入登录页，登录后返回该分享。已有登录用户直接保存到自己的图库，相册分享会创建独立相册。同一链接重复保存不会重复创建文件。
- 相册仅分享图片，最多 500 张；撤销或过期阻止后续访问，不影响已保存的副本。原图可能包含位置信息，创建链接前有明确提示。不会调用第三方短链或外部图片服务。
- 分享页图片沿用 Blob 加载、取消、懒加载和缩略图重试机制，公开请求不附带 Bearer 令牌，包含短链的 API 请求使用 `no-referrer`。支持移动端和中英文。

## 局部更新与请求调度

- 收藏、回收/恢复、相册/标签维护、分享链接创建/撤销和管理记录保存，优先使用写接口返回结果更新已有列表、详情及可推导统计，不再通过全局版本号重新加载整个工作区。
- `workspace.perform` 要求显式成功更新回调；`useAssetFeed` / `useRemoteData` 订阅有类型的局部变更。确实缺少数据时按资源范围失效，批量请求合并，失活页面延迟到再次进入时读取。
- `useVisiblePolling` 尊重自动刷新设置与页面可见性，防止轮询重叠；任务页不再与布局重复轮询统计。缩略图有会话隔离的内存缓存，最多 80 项 / 24 MiB / 60 秒，退出清空；公开分享不复用该缓存。
- 图库通过现有 `vue-virtual-scroller` 虚拟渲染瀑布流和列表，按实际容器宽度分列并保留图片比例；KeepAlive 最多缓存 6 页。相册/标签目录有 30 秒会话内缓存，同键读取及私有缩略图的并发请求共享底层请求，最后一个消费者取消才中止。
- 标签批量关联/解除改为单次事务请求，返回后一次发布局部变更；相册详情读取纯摘要、标签详情读取单个标签。上传合并等待使用轻量进度接口，需要续传时仍完整核对分片。新前端发布前需先部署配套服务端。
- 首次加载、筛选、分页、异步处理结果和未知的服务端筛选仍保留必要查询；局部更新不替代权限校验或跨设备同步。接口配套变化和剩余性能问题见 [性能优化清单](../../docs/性能优化.md)。
...
| `/settings`                   | 深浅/系统主题、网格密度、文件名显示、动画、自动刷新；只读服务器能力与存储统计；昵称与头像编辑、账户安全操作            |
...
- 上传先 `POST /uploads/sessions`；不超过 5 MiB 的文件以原始字节 `PUT /uploads/sessions/:id/content`，更大的文件按 5 MiB 分片上传，最后请求合并。最多同时上传两个文件，每个文件最多三个分片并发；图片支持 JPEG/JFIF/PJPEG/PJP、PNG/APNG、WebP、GIF、AVIF、SVG，单张 ≤ 10 MiB；视频支持 MP4/MOV/MKV，单个 ≤ 512 MiB、4 小时。图片和视频不限制像素或分辨率，动图 ≤ 1000 帧；扩展名不区分大小写，服务器校验实际内容。SVG 限于安全静态图形与内联样式，文件大小与解码超时保护仍保留。设置页将 `upload.maxPixels: null` 显示为「不限制」。
- 在相册详情页点击上传、从上传队列继续选择文件或拖拽文件时，上传任务固定绑定发起时的相册，切换页面不会改变已排队任务的目标。上传完成（包括秒传和续传）后复用相册关联接口，直接用返回的相册摘要更新数量和封面，不额外重查相册。关联失败保留已上传的资源 ID，重试只重新关联、不重算指纹或重传文件；停止关联不会删除图库文件。相册内上传同时需要 `upload:create` 和 `asset:category` 权限。
- 媒体详情的「查看原图片 / 查看原视频」打开原文件蒙层，底部提供放大、缩小、适应窗口、1:1 和下载；图片支持滚轮缩放、拖动平移，按 Esc 关闭。GIF/APNG 直接展示原始动画，SVG 只用 `<img>` 渲染。
- 媒体详情名称旁和图库/相册单选工具栏提供「重命名」，需要 `asset:edit` 权限，回收站内不提供此操作。弹窗只编辑不含扩展名的部分，通过 `PATCH /assets/:id` 保存后同步列表、详情与文件名搜索结果；原文件、扩展名、相册关联和分享链接保持不变。
...
- 上传异常时通过 `GET /uploads/sessions/:id` 查询最终状态；重试前再次核对，已完成的会话不重复上传，分片上传跳过已完整保存的分片。上传队列只存在内存；同一账号在会话创建后的 24 小时内重选内容、大小和文件名一致的文件可恢复未取消的有效分片会话，已完成的文件可按内容指纹复用。刷新后需要进入目标相册重新选择文件，才能继续完成该相册的关联。
...
- 设置中的外观与显示项保存在浏览器，除主题为浏览器共享外，偏好按账户保存，不冒充服务器配置。任务首页在可见标签页每 10 秒轮询；用户加载更多后不自动覆盖列表。侧栏统计每 30 秒刷新，可在设置关闭；关闭轮询不会暂停后端工作。
- 「账户与安全 → 编辑个人资料」通过 `PATCH /user/me` 保存昵称和头像，立即同步顶栏及本地登录资料，刷新后仍保留。昵称去除首尾空白后为 1–80 个字符；头像支持不超过 5 MiB、2000 万像素的 PNG/JPEG/WebP，在浏览器居中裁剪至最大 256px 并压缩。不支持 Canvas WebP 编码的浏览器（如部分 Safari）会改用 JPEG，避免静默退回 PNG 导致头像数据过大。保存时后端再次校验和转码，不请求外部头像地址。取消编辑不会提交，昵称单独修改不会覆盖头像。邮箱用于登录，需联系管理员修改；角色、权限不可通过此表单修改。
