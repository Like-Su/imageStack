...
- `src/components/workspace`：248px 侧栏、顶栏搜索、移动端导航、页面标题、原生模态框、提示和上传队列。
...
- 浏览器加载缩略图和下载原图时通过带鉴权的 `fetch` 获取 Blob，再生成临时对象 URL；不把令牌放进图片 URL。图片进入可见区域才请求，离开附近视口或卸载会取消请求并撤销对象 URL。HTTP 202 按 `Retry-After` 有限重试，超出等待后可刷新或重新进入视口。
- 上传先 `POST /uploads/sessions`，再以原始 File 字节 `PUT /uploads/sessions/:id/content`，不是 multipart 或 JSON。最多两个并发，接受 JPEG/PNG/WebP、单张不超过 10 MiB/2000 万像素，服务器仍做最终校验。
...
- 语义模型、OCR、自动人脸识别、AI 相册推荐、视频、S3、SSO、任务通知和自动备份未接入，页面明确说明并禁用相关开关；可用的替代能力是关键词搜索和手动人物/标签整理。
- 设置中的可写项都是浏览器偏好，不冒充服务器配置。除主题为浏览器共享外，偏好按账户保存。任务首页在可见标签页每 10 秒轮询；用户加载更多后不自动覆盖列表。侧栏统计每 30 秒刷新，可在设置关闭；关闭轮询不会暂停后端工作。
- 存储量仅为当前用户数据库中的原图大小，包含单独标识的回收站用量，不代表磁盘总容量、NAS 状态、其他用户或派生图用量。
...
认证和工作区均接入真实接口，不模拟邮件发送、登录、重置或媒体操作成功。媒体工作区不加载第三方示例图片或地图；原有认证展示区继续使用 Picsum 外部图片，完全离线部署时需替换为本地资源。没有新增数据库模型、迁移或依赖。

按需求仅做静态核对，未启动项目、未执行测试、构建或类型检查，未发送实际邮件。
