...
- 视频通过私有临时文件流式接收与计算哈希，不将大视频整体放入 Node 内存。后台同样流式读取，生成最长边 256px WebP 封面和不超过 1920×1080 的 H.264/AAC MP4 兼容预览；若预览超出 512 MiB、超时或解码失败，任务会重试/失败，原文件仍可下载。
...
- Worker 对受支持的图片生成最长边 256px 的 sm WebP，自动纠正方向且不放大小图；GIF/APNG 等动图取首帧，视频生成封面与兼容预览。
